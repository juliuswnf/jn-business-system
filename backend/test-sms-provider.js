...
if (result.error) {
  console.error('âŒ Error loading .env file:', result.error);
  process.exit(1);
}

console.log('âœ… .env file loaded from:', join(__dirname, '.env'));
console.log('');
...
console.log('ðŸ”§ SMS Provider Configuration Test\n');
console.log('Environment Variables:');
console.log('  SMS_PROVIDER:', process.env.SMS_PROVIDER || 'NOT SET');
console.log('  TWILIO_ACCOUNT_SID:', process.env.TWILIO_ACCOUNT_SID ? 'âœ… SET' : 'âŒ NOT SET');
console.log('  TWILIO_AUTH_TOKEN:', process.env.TWILIO_AUTH_TOKEN ? 'âœ… SET' : 'âŒ NOT SET');
console.log('  TWILIO_PHONE_NUMBER:', process.env.TWILIO_PHONE_NUMBER || 'NOT SET');
console.log('  MESSAGEBIRD_API_KEY:', process.env.MESSAGEBIRD_API_KEY ? 'âœ… SET' : 'âŒ NOT SET');
console.log('');

try {
  const provider = SMSProviderFactory.getProvider();
  
  console.log('âœ… Active SMS Provider:', provider.getName().toUpperCase());
  console.log('âœ… Provider Available:', provider.isAvailable());
  console.log('');
  ...
  console.log('ðŸŽ‰ SMS Provider configuration is valid!\n');
