...
async function verifyRecordCounts() {
  ...
  console.log('');
  if (salonCount === 0) logError('No salons found! Database may be empty.');
  if (userCount === 0) logError('No users found! Database may be empty.');
  const salonIds = await Salon.find().distinct('_id');
  const userIds = await User.find().distinct('_id');
  return { salonIds, userIds };
}

async function verifyReferences(salonIds, userIds) {
  console.log('🔎 Verifying salon references...');
  const invalidSalon = await Booking.countDocuments({ salon: { $nin: salonIds, $ne: null } });
  if (invalidSalon > 0) logError(`Found ${invalidSalon} bookings with invalid salon references`);
  else logSuccess('All booking salon references are valid');

  console.log('🔎 Verifying user references...');
  const invalidCustomer = await Booking.countDocuments({ customer: { $nin: userIds, $ne: null } });
  if (invalidCustomer > 0) logWarning(`Found ${invalidCustomer} bookings with missing customer references (may be anonymous)`);
  else logSuccess('All booking customer references are valid');
}
...
async function checkDuplicateKeys() {
  ...
  const duplicateKeys = await Booking.aggregate([
    ...
    { $group: { _id: '$idempotencyKey', count: { $sum: 1 }, bookings: { $push: '$_id' } } },
    { $match: { count: { $gt: 1 } } }
  ]);
  if (duplicateKeys.length > 0) {
    logError(`Found ${duplicateKeys.length} duplicate idempotency keys:`);
    duplicateKeys.forEach(dup => console.error(`  - Key: ${dup._id} (${dup.count} bookings)`));
  } else {
...
async function verifyRequiredFields() {
  ...
  if (salonsWithoutName === 0 && salonsWithoutSlug === 0) logSuccess('All salons have required fields');
  if (bookingsWithoutDate === 0 && bookingsWithoutTime === 0) logSuccess('All bookings have required fields');
}
...
async function verifyBackup() {
  try {
    ...
    const { salonIds, userIds } = await verifyRecordCounts();
    await verifyReferences(salonIds, userIds);
    ...
    const salonsWithoutOwner = await Salon.countDocuments({ owner: { $nin: userIds } });
    if (salonsWithoutOwner > 0) logWarning(`${salonsWithoutOwner} salons have missing owner references`);
    else logSuccess('All salons have valid owners');
    ...
    bookingsByStatus.forEach(stat => console.log(`    - ${stat._id}: ${stat.count}`));
