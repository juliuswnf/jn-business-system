    ...
    try {
      ...
      if (response.data.stats) {
        setStats({
          ...
          thisMonth: 0, // Would need separate API
          pending: 0,
          refunded: 0
        });
      }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-secondary/50 border border-accent/20 p-4 rounded-lg">
          ...
          <p className="text-3xl font-bold text-red-500">€{stats.refunded.toFixed(2)}</p>
        </div>
      </div>
              ...
              {payments.length === 0 ? (
                ...
                payments.map((payment) => (
                  <tr key={payment._id} className="border-t border-accent/10 hover:bg-accent/5 transition">
                    ...
                    <td className="px-6 py-4 text-slate-350">{payment.customerName || 'N/A'}</td>
                    <td className="px-6 py-4 text-white font-semibold">€{(payment.amount || 0).toFixed(2)}</td>
                    <td className="px-6 py-4 text-slate-350 capitalize">{payment.paymentMethod || 'Card'}</td>
