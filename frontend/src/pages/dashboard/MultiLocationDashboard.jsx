    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {dashboard && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white border border-zinc-200 rounded-xl p-6">
            <p className="text-zinc-600 text-sm mb-1">Standorte</p>
            <p className="text-3xl font-bold text-zinc-900">{dashboard.overview.locationCount}</p>
          </div>
          <div className="bg-white border border-zinc-200 rounded-xl p-6">
            <p className="text-zinc-600 text-sm mb-1">Termine heute</p>
            <p className="text-3xl font-bold text-blue-500">{dashboard.overview.todayBookings}</p>
          </div>
          <div className="bg-white border border-zinc-200 rounded-xl p-6">
            <p className="text-zinc-600 text-sm mb-1">Buchungen diesen Monat</p>
            <p className="text-3xl font-bold text-green-500">{dashboard.overview.monthlyBookings}</p>
          </div>
          <div className="bg-white border border-zinc-200 rounded-xl p-6">
            <p className="text-zinc-600 text-sm mb-1">Umsatz diesen Monat</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {locations.map(location => {
          ...
          return (
            <div key={location.id} className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    {location.address?.city && (
                      <p className="text-sm text-zinc-600">{location.address.city}</p>
                    )}
                  </div>
                ...
                {locationData && (
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      <p className="text-2xl font-bold text-zinc-900">{locationData.bookings}</p>
                      <p className="text-xs text-zinc-500">Buchungen</p>
                    </div>
                    <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-zinc-50 hover:bg-zinc-100 text-zinc-900 py-2 px-4 rounded-lg text-sm transition"
                  >
                    Öffnen
                  </button>
                  {!location.isPrimary && (
        ...
        {locations.length < 5 && (
          <button
            onClick={() => setShowAddModal(true)}
            className="bg-white/50 border border-dashed border-zinc-200 rounded-xl p-6 flex flex-col items-center justify-center min-h-[200px] hover:border-zinc-300 transition"
          >
            ...
            <p className="text-zinc-600">Standort hinzufügen</p>
            <p className="text-xs text-zinc-500 mt-1">{5 - locations.length} verfügbar</p>
          </button>
        )}
      </div>
      ...
      {dashboard?.byLocation && dashboard.byLocation.length > 1 && (
        <div className="bg-white border border-zinc-200 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-zinc-900 mb-4">Standort-Vergleich</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-200">
                  <th className="text-left py-3 px-4 text-zinc-600 font-medium">Standort</th>
                  <th className="text-right py-3 px-4 text-zinc-600 font-medium">Buchungen</th>
                  <th className="text-right py-3 px-4 text-zinc-600 font-medium">Abgeschlossen</th>
                  <th className="text-right py-3 px-4 text-zinc-600 font-medium">Storniert</th>
                  <th className="text-right py-3 px-4 text-zinc-600 font-medium">Umsatz</th>
                </tr>
              </thead>
              <tbody>
                {dashboard.byLocation.map((loc, i) => (
                  <tr key={i} className="border-b border-zinc-200/50">
                    <td className="py-3 px-4 text-zinc-900 font-medium">{loc.name}</td>
                    <td className="py-3 px-4 text-right text-zinc-900">{loc.bookings}</td>
                    ...
                    <td className="py-3 px-4 text-right text-zinc-900 font-medium">€{loc.revenue.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-white/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-zinc-200 rounded-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-zinc-900">Neuen Standort hinzufügen</h2>
              <button onClick={() => setShowAddModal(false)} className="text-zinc-600 hover:text-zinc-900">
            ...
            <form onSubmit={handleAddLocation} className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-zinc-600 mb-2">E-Mail</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-sm text-zinc-600 mb-2">Geschäftsart</label>
                <select
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 bg-zinc-50 hover:bg-zinc-100 text-zinc-900 py-2 rounded-lg transition"
                >
                  Abbrechen
                </button>
                <button
                  type="submit"
                  disabled={adding}
                  className="flex-1 bg-red-600 hover:bg-red-700 disabled:bg-gray-600 text-zinc-900 py-2 rounded-lg font-medium transition"
                >
