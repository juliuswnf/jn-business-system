    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-gray-50 border border-gray-200 p-6 rounded-xl">
          ...
          <p className="text-2xl font-semibold tracking-tight text-gray-900">{bookings.filter(b => new Date(b.bookingDate) > new Date()).length}</p>
        </div>

        <div className="bg-gray-50 border border-gray-200 p-6 rounded-xl">
          <div className="flex items-center gap-3 mb-2">
            <Clock className="text-gray-600" size={24} />
            <h3 className="font-semibold text-gray-900">Vergangene Termine</h3>
          </div>
          <p className="text-2xl font-semibold tracking-tight text-gray-900">{bookings.filter(b => new Date(b.bookingDate) <= new Date()).length}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-900">Ihre Termine</h2>

        {bookings.length === 0 ? (
          <div className="text-center py-12">
            <Calendar className="mx-auto text-gray-500 mb-4" size={48} />
            <h3 className="text-lg font-medium text-gray-600 mb-2">Keine Termine gefunden</h3>
            ...
            {bookings.map((booking) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900">{booking.serviceId?.name || 'Service'}</h4>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-900">Kontoinformationen</h2>
