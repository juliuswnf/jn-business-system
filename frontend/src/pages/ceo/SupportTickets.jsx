...
const SupportTickets = () => {
  ...
  const getPriorityBadge = (priority) => {
    const styles = {
      ...
      medium: 'bg-gray-50 text-gray-500',
    ...
    return <span className={`px-2 py-1 rounded text-xs font-medium ${styles[priority] || 'bg-gray-500/20 text-gray-500'}`}>{labels[priority] || priority}</span>;
  };
          ...
          <div className="flex items-center justify-between">
            <Link to="/ceo/dashboard" className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Kundenanfragen verwalten</p>
              </div>
            </Link>
            <UserMenu />
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
            <p className="text-gray-400 text-sm mb-1">Gesamt Tickets</p>
          ...
          <div className="bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
            ...
            <p className="text-2xl font-semibold tracking-tight text-red-600">{stats.open}</p>
          </div>
          <div className="bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
          ...
          <div className="bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
            <p className="text-gray-400 text-sm mb-1">Gelöst</p>
            <p className="text-2xl font-semibold tracking-tight text-green-600">{stats.resolved}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {[
            ...
            <button key={key} onClick={() => setFilter(key)} className={`px-4 py-2 rounded-lg whitespace-nowrap transition ${filter === key ? 'bg-orange-500 text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Tickets List */}
          <div className="lg:col-span-1 bg-white/50 border border-gray-200 rounded-xl overflow-hidden">
            ...
            {tickets.length === 0 ? (
              ...
              <div className="divide-y divide-gray-200 max-h-[600px] overflow-y-auto">
                {tickets.map((ticket) => (
                  <button key={ticket._id} onClick={() => setSelectedTicket(ticket)} className={`w-full text-left p-4 hover:bg-gray-100/50 transition ${selectedTicket?._id === ticket._id ? 'bg-gray-50/70' : ''}`}>
          ...
          {/* Ticket Details */}
          <div className="lg:col-span-2 bg-white/50 border border-gray-200 rounded-xl overflow-hidden">
            {selectedTicket ? (
              <>
                <div className="p-4 border-b border-gray-200">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">#{selectedTicket.ticketNumber || selectedTicket._id.slice(-6)} • {selectedTicket.customerEmail || selectedTicket.salonName}</p>
                    </div>
                ...
                <div className="p-4 space-y-4 max-h-[400px] overflow-y-auto">
                  {selectedTicket.messages?.length > 0 ? (
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <p className="text-gray-600">{selectedTicket.description || 'Keine Nachricht'}</p>
                    </div>
                  )}
                </div>
                ...
                {selectedTicket.status !== 'closed' && (
                  <div className="p-4 border-t border-gray-200">
                    <div className="flex gap-2 mb-4">
                      <select onChange={(e) => updateStatus(selectedTicket._id, e.target.value)} value={selectedTicket.status} className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-sm">
                    ...
                    <textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder="Antwort schreiben..." className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 resize-none" rows={3} />
                    <button onClick={sendReply} disabled={sending || !replyText.trim()} className="mt-3 px-6 py-2 bg-gray-900 text-gray-900 rounded-lg hover:bg-gray-800 transition disabled:opacity-50 flex items-center gap-2">
                      {sending ? (
                        <>
