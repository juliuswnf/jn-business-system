...
const Analytics = () => {
  ...
  const StatCard = ({ title, value, change, changeLabel, icon, color }) => (
    <div className="bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
      ...
      <p className="text-2xl font-semibold tracking-tight text-gray-900 mb-1">{value}</p>
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <Link to="/ceo/dashboard" className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900">Analytics Deep-Dive</h1>
                <p className="text-xs text-gray-400">Wachstum & Performance</p>
              </div>
            </Link>
            <div className="flex items-center gap-4">
              <select value={timeRange} onChange={(e) => setTimeRange(e.target.value)} className="px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-sm">
                <option value="7d">Letzte 7 Tage</option>
                <option value="30d">Letzte 30 Tage</option>
                <option value="90d">Letzte 90 Tage</option>
                <option value="1y">Letztes Jahr</option>
              </select>
              <button onClick={fetchAllAnalytics} className="p-2 text-gray-500 hover:text-gray-900 transition" title="Aktualisieren">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-2 bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
              ...
              <div className="h-64 flex items-end gap-4">
                {revenueChart.map((item, idx) => (
                  <div key={idx} className="flex-1 flex flex-col items-center">
                    ...
                    <p className="text-gray-900 text-sm font-medium">{formatCurrency(item.revenue)}</p>
                  </div>
                ))}
              </div>
            ) : <div className="h-64 flex items-center justify-center text-gray-400">Keine Daten verfügbar</div>}
          </div>

          <div className="bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-6">Kündigungsgründe</h3>
            ...
            ) : <div className="h-48 flex items-center justify-center text-gray-400 text-center">Keine Kündigungen 🎉</div>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white/50 border border-gray-100 rounded-2xl shadow-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 bg-yellow-500/10 rounded-lg border border-yellow-500/30">
                <span className="text-gray-900">Testphase</span>
                <div className="text-right">
                  <p className="text-yellow-600 font-bold">{overview?.subscriptions?.trial || 0}</p>
                  <p className="text-gray-400 text-sm">Potenzielle Kunden</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                <p className="text-gray-500 text-sm mb-1">Annual Recurring Revenue</p>
                <p className="text-2xl font-semibold tracking-tight text-gray-900">{formatCurrency(overview?.revenue?.arr || 0)}</p>
              </div>
              <div>
                <p className="text-gray-500 text-sm mb-1">Ø Revenue pro Kunde</p>
                <p className="text-green-600 font-medium">{formatCurrency(overview?.revenue?.arpu || 0)}/Monat</p>
              </div>
              <div className="pt-4 border-t border-gray-200">
                <p className="text-gray-500 text-sm mb-1">Neue Kunden (Zeitraum)</p>
                <p className="text-gray-900 font-medium">+{overview?.overview?.newCustomers || 0}</p>
              </div>
            </div>
          </div>
        </div>
