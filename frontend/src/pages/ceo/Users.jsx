...
export default function Users() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight text-gray-900 mb-2">Benutzerverwaltung</h1>
        <p className="text-gray-400">Alle Benutzer der Plattform verwalten</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-secondary/50 border border-gray-200 p-4 rounded-lg">
          <p className="text-gray-400 text-sm mb-2">Gesamt</p>
          <p className="text-2xl font-semibold tracking-tight text-accent">{counts.total}</p>
        </div>
        <div className="bg-secondary/50 border border-gray-200 p-4 rounded-lg">
          <p className="text-gray-400 text-sm mb-2">Unternehmer</p>
          <p className="text-2xl font-semibold tracking-tight text-gray-600">{counts.salon_owners}</p>
        </div>
        <div className="bg-secondary/50 border border-gray-200 p-4 rounded-lg">
          <p className="text-gray-400 text-sm mb-2">Mitarbeiter</p>
          <p className="text-2xl font-semibold tracking-tight text-green-500">{counts.employees}</p>
        </div>
        <div className="bg-secondary/50 border border-gray-200 p-4 rounded-lg">
          <p className="text-gray-400 text-sm mb-2">Kunden</p>
          <p className="text-2xl font-semibold tracking-tight text-purple-500">{counts.customers}</p>
        </div>
      </div>
      ...
      <div className="bg-secondary/50 border border-gray-200 rounded-lg overflow-hidden">
        <form onSubmit={handleSearch} className="p-6 border-b border-gray-200">
          <div className="flex gap-4">
            ...
            <button type="submit" className="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-900/80">
              Suchen
            </button>
          </div>
        </form>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">Unternehmen</th>
              ...
              {users.length === 0 ? (
                ...
                users.map((user) => (
                  <tr key={user.id} className="border-t border-gray-200 hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 text-gray-400">{user.companyName || '-'}</td>
                    <td className="px-6 py-4">
