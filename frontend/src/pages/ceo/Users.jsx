...
export default function Users() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-zinc-900 mb-2">Benutzerverwaltung</h1>
        <p className="text-zinc-400">Alle Benutzer der Plattform verwalten</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-secondary/50 border border-zinc-200 p-4 rounded-lg">
          <p className="text-zinc-400 text-sm mb-2">Gesamt</p>
          <p className="text-3xl font-bold text-accent">{counts.total}</p>
        </div>
        <div className="bg-secondary/50 border border-zinc-200 p-4 rounded-lg">
          <p className="text-zinc-400 text-sm mb-2">Unternehmer</p>
          <p className="text-3xl font-bold text-blue-500">{counts.salon_owners}</p>
        </div>
        <div className="bg-secondary/50 border border-zinc-200 p-4 rounded-lg">
          <p className="text-zinc-400 text-sm mb-2">Mitarbeiter</p>
          <p className="text-3xl font-bold text-green-500">{counts.employees}</p>
        </div>
        <div className="bg-secondary/50 border border-zinc-200 p-4 rounded-lg">
          <p className="text-zinc-400 text-sm mb-2">Kunden</p>
      ...
      <div className="bg-secondary/50 border border-zinc-200 rounded-lg overflow-hidden">
        <form onSubmit={handleSearch} className="p-6 border-b border-zinc-200">
          <div className="flex gap-4">
            ...
            <button type="submit" className="px-4 py-2 bg-zinc-900 text-white rounded-lg hover:bg-zinc-900/80">
              Suchen
            </button>
          </div>
        </form>
          ...
          <table className="w-full">
            <thead className="bg-zinc-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-zinc-900">Rolle</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-zinc-900">Unternehmen</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-zinc-900">Status</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-zinc-900">Aktionen</th>
              </tr>
            </thead>
            <tbody>
              {users.length === 0 ? (
                ...
                users.map((user) => (
                  <tr key={user.id} className="border-t border-zinc-200 hover:bg-zinc-50 transition">
                    <td className="px-6 py-4 text-zinc-900 font-semibold">{user.name}</td>
                    ...
                    <td className="px-6 py-4 text-zinc-400">{user.companyName || '-'}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        !user.banned ? 'bg-green-500/20 text-green-600' : 'bg-red-500/20 text-red-600'
                      }`}>
                    ...
                    <td className="px-6 py-4">
                      {user.role !== 'ceo' && (
                        <button
                          ...
                          className={`text-sm font-semibold ${user.banned ? 'text-green-600 hover:text-green-600' : 'text-red-600 hover:text-red-600'}`}
                        >
