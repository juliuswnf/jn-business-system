...
const SERVICE_TEMPLATES = [
  {
    label: 'Barbershop',
    services: [
      { name: 'Herrenschnitt', duration: 30, price: 25 },
      { name: 'Bart trimmen', duration: 20, price: 15 },
      { name: 'Rasur', duration: 30, price: 20 },
      { name: 'Kinderschnitt', duration: 20, price: 18 },
    ],
  },
  {
    label: 'Friseursalon',
    services: [
      { name: 'Damenschnitt', duration: 60, price: 45 },
      { name: 'Herrenschnitt', duration: 30, price: 25 },
      { name: 'Färben', duration: 120, price: 80 },
      { name: 'Strähnen', duration: 90, price: 70 },
    ],
  },
  {
    label: 'Nagelstudio',
    services: [
      { name: 'Maniküre', duration: 45, price: 35 },
      { name: 'Pediküre', duration: 60, price: 40 },
      { name: 'Gel-Nägel', duration: 90, price: 60 },
      { name: 'Shellac', duration: 60, price: 45 },
    ],
  },
  {
    ...
    services: [
      ...
      { name: 'Piercing', duration: 30, price: 30 },
      { name: 'Tattoo Klein', duration: 60, price: 80 },
      { name: 'Tattoo Groß', duration: 180, price: 200 },
    ],
  },
];
...
<script src="${baseUrl}/widget.js" async></script>`;
  ...
  const useTemplate = (template) => {
    const tpl = SERVICE_TEMPLATES.find(t => t.label === template);
    if (tpl) {
      setServices(tpl.services.map(s => ({ ...s, enabled: true })));
    }
  };
          ...
          {currentStep === 4 && (
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Branchenvorlage wählen</p>
                <div className="grid grid-cols-2 gap-2">
                  {SERVICE_TEMPLATES.map(tpl => (
                    <button
                      key={tpl.label}
                      onClick={() => useTemplate(tpl.label)}
                      className="px-3 py-2 bg-white border border-gray-200 rounded-xl text-gray-700 hover:border-gray-400 hover:text-gray-900 transition text-sm font-medium text-left"
                    >
                      {tpl.label}
                    </button>
                  ))}
