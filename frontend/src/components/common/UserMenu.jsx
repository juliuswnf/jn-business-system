...
const UserMenu = () => {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        {/* User Avatar */}
        <div className="w-9 h-9 bg-zinc-50 border border-zinc-200 rounded-full flex items-center justify-center text-zinc-900 text-sm font-semibold">
          {user?.name?.charAt(0)?.toUpperCase() || 'U'}
        </div>
        
        {/* Arrow - only shows on hover or when menu is open */}
        <svg 
          className={`w-4 h-4 text-zinc-500 transition-all duration-200 ${
      ...
      {showMenu && (
        <>
          ...
          {/* Menu */}
          <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl border border-zinc-200 shadow-none z-50 overflow-hidden">
            {/* User Info */}
            ...
            <div className="py-1">
              <Link
                to="/"
                onClick={() => setShowMenu(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-600 hover:bg-white/5 hover:text-zinc-900 transition"
              >
