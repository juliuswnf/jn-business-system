...
const UserMenu = () => {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        {/* User Avatar */}
        <div className="w-9 h-9 bg-zinc-600 border border-zinc-500 rounded-full flex items-center justify-center text-white text-sm font-semibold">
          {user?.name?.charAt(0)?.toUpperCase() || 'U'}
        </div>
        
        {/* Arrow - only shows on hover or when menu is open */}
        <svg 
          className={`w-4 h-4 text-zinc-300 transition-all duration-200 ${
      ...
      {showMenu && (
        <>
          ...
          {/* Menu */}
          <div className="absolute right-0 mt-2 w-56 bg-zinc-700 rounded-xl border border-zinc-600 shadow-xl z-50 overflow-hidden">
            {/* User Info */}
