    ...
    <div className="fixed inset-0 z-[9999]">
      ...
      {targetRect && step.highlight && (
        <>
          {/* Highlight box */}
          <div
            className="absolute border-2 border-gray-200 rounded-xl transition-all duration-300 pointer-events-none"
            style={{
          ...
          {/* Pulse animation */}
          <div
            className="absolute border-2 border-gray-200 rounded-xl animate-ping pointer-events-none"
            style={{
              top: targetRect.top - 4,
              left: targetRect.left - 4,
              width: targetRect.width + 8,
      ...
      {/* Tooltip */}
      <div
        className="absolute bg-white border border-gray-200 rounded-2xl shadow-sm p-6 w-[360px] max-w-[90vw] transition-all duration-300"
        style={getTooltipStyle()}
      >
        {/* Progress bar */}
        <div className="absolute top-0 left-0 right-0 h-1 bg-gray-50 rounded-t-2xl overflow-hidden">
          <div
        ...
        {step.action && (
          <button
            ...
            className="w-full mb-4 py-2.5 bg-gray-500/20 border border-gray-200/50 text-gray-900 rounded-xl hover:bg-gray-500/30 transition-colors text-sm font-medium"
          >
            {step.action.label} →
          </button>
        )}
        ...
        <div className="flex gap-3">
          {!isFirstStep && (
            <button
              onClick={handlePrev}
              className="flex-1 py-2.5 bg-gray-50 text-gray-600 rounded-xl hover:bg-gray-100 transition-colors font-medium"
            >
              Zurück
            </button>
          )}
          <button
            onClick={handleNext}
            className="flex-1 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-gray-900 rounded-xl hover:opacity-90 transition-opacity font-medium"
          >
      ...
      <button
        onClick={handleSkip}
        className="absolute top-4 right-4 w-10 h-10 rounded-full bg-gray-50/80 text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors flex items-center justify-center"
      >
