...
export default function OnboardingTour({ onComplete, onSkip }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999]">
      ...
      {/* Tooltip */}
      <div
        className="absolute bg-white border border-zinc-200 rounded-2xl shadow-none p-6 w-[360px] max-w-[90vw] transition-all duration-300"
        style={getTooltipStyle()}
      >
        {/* Progress bar */}
        <div className="absolute top-0 left-0 right-0 h-1 bg-zinc-50 rounded-t-2xl overflow-hidden">
          <div
        ...
        <p className="text-zinc-600 leading-relaxed mb-6">{step.content}</p>

        {/* Action button (optional) */}
        {step.action && (
          <button
            onClick={handleAction}
            className="w-full mb-4 py-2.5 bg-indigo-500/20 border border-indigo-500/50 text-zinc-900 rounded-lg hover:bg-indigo-500/30 transition-colors text-sm font-medium"
          >
            {step.action.label} →
          </button>
        )}
        ...
        <div className="flex gap-3">
          {!isFirstStep && (
            <button
              onClick={handlePrev}
              className="flex-1 py-2.5 bg-zinc-50 text-zinc-600 rounded-lg hover:bg-zinc-100 transition-colors font-medium"
            >
              Zurück
            </button>
          )}
          <button
            onClick={handleNext}
            className="flex-1 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-zinc-900 rounded-lg hover:opacity-90 transition-opacity font-medium"
          >
      ...
      <button
        onClick={handleSkip}
        className="absolute top-4 right-4 w-10 h-10 rounded-full bg-zinc-50/80 text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 transition-colors flex items-center justify-center"
      >
